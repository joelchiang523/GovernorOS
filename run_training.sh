...
#   4. self-optimize（依 benchmark / strategy 歷史調整門檻）
#   5. evening startup（sleep + deep + decay）→ 知識固化進 L4/L5
#   6. 匯出訓練資料快照
...
if [ "$SKIP_MORNING" = false ]; then
  echo ""
echo "[Step 1/6] Morning startup（decay + score-strategies + self-opt + wake）..."
  bash "$SCRIPT_DIR/startup.sh" morning
else
  echo "[Step 1/6] Morning startup 已略過"
fi

# ── Step 2: Reset tasks ─────────────────────────────────
echo ""
echo "[Step 2/6] 重置 benchmark tasks 到初始狀態..."
bash "$RESET_SCRIPT"

# ── Step 3: 執行 benchmark suite ───────────────────────
echo ""
echo "[Step 3/6] 執行 coding_suite_v2（10 題）..."
echo "  使用模型：$MODEL"
...
echo ""
echo "[Step 4/6] Self-Optimize（依最新 benchmark / strategy 調整門檻）..."
$PYTHON "$SCRIPT_DIR/dream_cycle.py" --self-optimize --apply

# ── Step 5: Evening startup ─────────────────────────────
if [ "$SKIP_EVENING" = false ]; then
  echo ""
  echo "[Step 5/6] Evening startup（sleep + deep + decay）→ L3→L4→L5 固化..."
  bash "$SCRIPT_DIR/startup.sh" evening
else
  echo "[Step 5/6] Evening startup 已略過"
fi

# ── Step 6: 匯出訓練資料 ────────────────────────────────
echo ""
echo "[Step 6/6] 匯出訓練資料快照..."
