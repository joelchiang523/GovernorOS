...
# ── 鎖定（防止重複執行或與 run_training.sh 競爭 GPU） ────
if [ -f "$LOCK" ]; then
...
# 額外偵測：benchmark_runner 或 main_loop 仍在跑（防止 run_training.sh 手動觸發時競爭 GPU）
if pgrep -f "benchmark_runner.py|main_loop.py" > /dev/null 2>&1; then
    echo "[SKIP] 偵測到其他訓練程序（benchmark_runner/main_loop）正在執行，跳過本次"
    exit 0
fi
echo $$ > "$LOCK"
trap 'rm -f "$LOCK"' EXIT
